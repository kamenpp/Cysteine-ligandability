...
def graphs_from_pdb_pocket(pdb_pocket, key, label, cys, graph_dic_path, path_marker='/', EtaR=4.00, ShfR=0.5, Zeta=8.00, ShtZ=0):
    """
    will detect disulfide bridges and filter them out
    node features: AtomFeaturizer, TorchANI
    edge feature: BondFeaturizer, D3_info_cal,
    :param dir: the path for the PDB pocket file
    ...
    if not os.path.exists(graph_dic_path + path_marker + key + '.bin'):
        try:
            mol = Chem.MolFromPDBFile(pdb_pocket)
            
            for bond in mol.GetBonds():
                    ba = bond.GetBeginAtom()
                    ea = bond.GetEndAtom()
                    if ba.GetAtomicNum() == 16 and ea.GetAtomicNum() == 16:
                        print('FOUND DISULFIDE BOND for id ', pdb_pocket)
                        return
            # pocket graph
            g1 = dgl.DGLGraph()
            ...
            index = get_SG_index(mol,cys[0],int(cys[1]))
...
class GraphDatasetGenerateSingleThread(object):

    """
    This class is used for generating graph objects.
    """

    def __init__(self, keys=None, labels=None, cyss = None, graph_ls_path=None, graph_dic_path=None, name_of_data='data',
        ...
        :param name_of_data: training/validation/testing
        ...
        self.ShtZ = ShtZ
        self.spl = split
        self._pre_process(name_of_data)


    def _pre_process(self,name_of_data):
        if os.path.exists(self.graph_ls_path+self.path_marker+ name_of_data + '.bin' ):
            print('Loading previously saved dgl graphs and corresponding data...')
            with open(self.graph_ls_path+self.path_marker+name_of_data +'.bin', 'rb') as f:
                data = pickle.load(f)
            ...
            for key in tqdm(self.origin_keys):
                try:
                    ...
                    print('Could not open graph dict', self.graph_dic_path + self.path_marker + key + '.bin')
                    continue
            
            with open(self.graph_ls_path + self.path_marker + name_of_data +'.bin', 'wb') as f:
